...
namespace Relational2Rdf.Converter.Ontology
{
	public class OntologyConversionContext
	{
		...
		public IRI GetTypeIri(ISchema schema, ITable table) => GetTypeIri(schema.Name, table.Name);
		...
		public OntologyConversionContext(ITripletWriter writer, IRelationalDataSource source, OntologySettings settings)
		{
			...
			ColumnIris = source.Schemas.SelectMany(s => s.Tables.SelectMany(t => t.Columns.Select(c => (schema: s, table: t, column: c)))).ToFrozenDictionary(x => (x.schema.Name, x.table.Name, x.column.Name), x => TableIris[(x.schema.Name, x.table.Name)].Extend(HttpUtility.UrlEncode(x.column.Name)));
			ForeignKeyIris = source.Schemas.SelectMany(s => s.Tables.SelectMany(t => t.ForeignKeys.Select(k => (schema: s, table: t, key: k)))).ToFrozenDictionary(x => (x.schema.Name, x.table.Name, x.key.Name), x => TableIris[(x.schema.Name, x.table.Name)].Extend("foreignKey").Extend(HttpUtility.UrlEncode(x.key.Name)));
		}
	}
}
